...
import type { Summary, TimeseriesBucket, TimeseriesBucketSize, TimeseriesResponse } from '../api.ts';
import { type FormattedUsd, formatCount, formatUsd, parseMicros, plural } from '../format.ts';
import type { RangeId, RangeOption } from '../ranges.ts';
import { rangeOption } from '../ranges.ts';
import { computeTicks, type Ticks } from './scale.ts';

/** The bucket a Savings-view chart uses for `range` at the current width (DESIGN.md §12.2 table). */
export function bucketForRange(range: RangeId, narrow: boolean): TimeseriesBucketSize {
  if (range === '1h') return narrow ? '5m' : '1m';
  if (range === '7d') return narrow ? '1d' : '1h';
  if (range === '24h') return '1h';
  return '1d';
}

/** The word the chart titles use for one bucket: "Saved per minute", "Spend per day (UTC)". */
export function bucketWord(bucket: TimeseriesBucketSize): string {
  switch (bucket) {
    case '1m':
      return 'minute';
    case '5m':
...
// ---------------------------------------------------------------- per-bucket marks (DESIGN.md §12.4)

export type SavingsMark =
  | { readonly kind: 'empty' }
  | { readonly kind: 'unknown' }
  | { readonly kind: 'zero' }
  /** `amount` is in dollars; negative when routing cost more than the baseline in this bucket. */
  | { readonly kind: 'value'; readonly amount: number };

export type SpendMark = { readonly kind: 'gap' } | { readonly kind: 'value'; readonly amount: number };

export interface SavingsBucket {
  readonly index: number;
  readonly start: Date;
  /** The newest bucket: still filling, marked "(in progress)". */
  readonly inProgress: boolean;
  readonly requests: number;
  readonly unknownSavingsRequests: number;
  /** Draw the left-out strip: requests happened, savings are known, but some were left out. */
  readonly leftOut: boolean;
  readonly savings: SavingsMark;
  readonly spend: SpendMark;
}

/** Classifies every bucket for drawing; a value of 'unknown' is never confused with a $0 value. */
export function classifyBuckets(buckets: readonly TimeseriesBucket[]): SavingsBucket[] {
  ...
  return buckets.map((bucket, index) => {
    ...
    let savings: SavingsMark;
    if (requests === 0) savings = { kind: 'empty' };
    else if (savingsMicros === null) savings = { kind: 'unknown' };
    else if (savingsMicros === 0) savings = { kind: 'zero' };
    else savings = { kind: 'value', amount: savingsMicros / 1_000_000 };
    const spend: SpendMark =
      requests === 0 || spendMicros === null ? { kind: 'gap' } : { kind: 'value', amount: spendMicros / 1_000_000 };
    return {
      index,
...
export function maxSavings(buckets: readonly SavingsBucket[]): number {
...
export function minSavings(buckets: readonly SavingsBucket[]): number {
...
export function maxSpend(buckets: readonly SavingsBucket[]): number {
  return buckets.reduce((max, b) => (b.spend.kind === 'value' ? Math.max(max, b.spend.amount) : max), 0);
}

// ---------------------------------------------------------------- spend line (DESIGN.md §12.3, §12.4)

/**
 * The spend line split at its gaps: every run of two or more consecutive buckets with known spend is a
 * polyline (bucket indexes, in order); every bucket with known spend and no known-spend neighbour on
 * either side is a lone dot, since a line needs two points and a lone moveto draws nothing.
 */
export interface SpendRuns {
  readonly lines: readonly (readonly number[])[];
  readonly dots: readonly number[];
}

export function spendRuns(buckets: readonly SavingsBucket[]): SpendRuns {
  const lines: number[][] = [];
  const dots: number[] = [];
  let run: number[] = [];
  const close = (): void => {
    if (run.length === 1 && run[0] !== undefined) dots.push(run[0]);
    else if (run.length > 1) lines.push(run);
    run = [];
  };
  for (const bucket of buckets) {
    if (bucket.spend.kind === 'value') run.push(bucket.index);
    else close();
  }
  close();
  return { lines, dots };
...
/** A point of the spend chart, in SVG user units. */
 ...
 */
export function spendGeometry(
  buckets: readonly SavingsBucket[],
  x: (index: number) => number,
  y: (amount: number) => number,
): { readonly d: string; readonly dots: readonly ChartPoint[] } {
  const pointOf = (index: number): ChartPoint => {
...
function hourMinute(date: Date, utc: boolean): string {
  const hours = utc ? date.getUTCHours() : date.getHours();
  const minutes = utc ? date.getUTCMinutes() : date.getMinutes();
  return `${pad2(hours)}:${pad2(minutes)}`;
}

function isoDate(date: Date, utc: boolean): string {
  if (utc) return date.toISOString().slice(0, 10);
 ...
 */
export function bucketStartText(date: Date, bucketSize: TimeseriesBucketSize, multiDay: boolean): string {
  if (bucketSize === '1d') return isoDate(date, true);
...
function shortDate(date: Date, utc: boolean): string {
  const month = utc ? date.getUTCMonth() : date.getMonth();
  const day = utc ? date.getUTCDate() : date.getDate();
  return `${MONTHS[month]} ${day}`;
}

function thinLabels(labels: readonly AxisLabel[], max: number): AxisLabel[] {
...
export function xAxisLabels(
  ...
  for (const entry of buckets) {
    ...
    if (matches) labels.push({ index: entry.index, text });
  }
  return narrow ? thinLabels(labels, 5) : labels;
}

// ---------------------------------------------------------------- readout (DESIGN.md §12.3)

export interface Readout {
  readonly time: string;
  readonly inProgress: boolean;
  readonly saved: FormattedUsd;
  readonly spend: FormattedUsd;
...
export function readoutAt(
  ...
  index: number,
  bucketSize: TimeseriesBucketSize = '1m',
  multiDay = false,
): Readout | undefined {
  ...
  const dash: FormattedUsd = { text: '—', exact: undefined, unknown: false };
  return {
...
export function defaultReadout(
  ...
  for (let index = buckets.length - 1; index >= 0; index -= 1) {
    if ((buckets[index]?.requests ?? 0) > 0) return readoutAt(buckets, index, bucketSize, multiDay);
  }
  return undefined;
}
...
export function savingsAriaLabel(
  range: RangeId,
  bucket: TimeseriesBucketSize,
  buckets: readonly SavingsBucket[],
  totalSavings: FormattedUsd,
  leftOutCount: number,
): string {
  const word = bucketWord(bucket);
  const span: RangeOption = rangeOption(range);
  ...
  const leftOutPart =
    leftOutCount > 0 ? ` ${plural(leftOutCount, 'request', 'requests')} left out: savings unknown.` : '';
  return `${head}${totalPart}${leftOutPart}`;
}

/** "Spend per minute over the last hour. $0.539 in total; highest $0.0216 in one minute." */
export function spendAriaLabel(
  range: RangeId,
  bucket: TimeseriesBucketSize,
  buckets: readonly SavingsBucket[],
  totalSpend: FormattedUsd,
): string {
  const word = bucketWord(bucket);
...
// ---------------------------------------------------------------- table alternative (DESIGN.md §12.6)

export interface TableRow {
  readonly start: string;
  readonly inProgress: boolean;
  readonly requests: string;
  /** undefined shows as the em dash "--" (no requests in the bucket). */
  readonly spend: string | undefined;
  readonly saved: string | undefined;
  readonly leftOut: string;
}

// ---------------------------------------------------------------- the whole chart's view (DESIGN.md §12)

export interface SavingsChartView {
  readonly bucket: TimeseriesBucketSize;
  readonly bucketWord: string;
  readonly buckets: readonly SavingsBucket[];
  readonly savingsTotalText: string;
  readonly spendTotalText: string;
  readonly savingsTicks: Ticks;
  readonly spendTicks: Ticks;
  readonly xLabels: readonly AxisLabel[];
  readonly readout: Readout | undefined;
  /** One readout per bucket, in draw order, for the hover readout. */
  readonly readouts: readonly (Readout | undefined)[];
  readonly savingsAriaLabel: string;
  readonly spendAriaLabel: string;
 ...
 */
export function buildSavingsChartView(
...
export function tableRows(
  buckets: readonly TimeseriesBucket[],
  bucketSize: TimeseriesBucketSize,
  multiDay = false,
): TableRow[] {
  const lastIndex = buckets.length - 1;
  return buckets.map((bucket, index) => {
