...
export function normalizeAccessKey(input: string): string | undefined {
  ...
  if (key === '' || !/^[\x21-\x7e](?:[\x20-\x7e]*[\x21-\x7e])?$/.test(key)) return undefined;
  return key;
}

/** The key stored for this tab, if any. A stored value that could not be sent is treated as none. */
export function readAccessKey(storage: KeyStorage | undefined): string | undefined {
  const stored = storage?.getItem(ACCESS_KEY_ENTRY);
  return stored === null || stored === undefined ? undefined : normalizeAccessKey(stored);
}

export function storeAccessKey(storage: KeyStorage | undefined, key: string): void {
  storage?.setItem(ACCESS_KEY_ENTRY, key);
}

export function forgetAccessKey(storage: KeyStorage | undefined): void {
...
export function apiHeaders(key: string | undefined, accept = 'application/json'): Record<string, string> {
