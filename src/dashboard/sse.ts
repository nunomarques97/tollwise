...
export interface EventStreamParser {
  /** Feeds the next chunk of text; returns every event it completed, in order. */
...
export function createEventStreamParser(): EventStreamParser {
  ...
  const takeLine = (line: string, done: StreamEvent[]): void => {
    if (line === '') {
      // A blank line ends an event; one without data is dropped, as the specification says.
      if (hasData) done.push({ event: eventName === '' ? 'message' : eventName, data: dataLines.join('\n') });
      eventName = '';
      dataLines = [];
      hasData = false;
      return;
    }
    if (line.startsWith(':')) return; // A comment, used as a keep-alive.
