...
import type { HealthSnapshot } from '../api.ts';
import { h, replaceChildren } from '../dom.ts';
import { buildProvidersView, type ProviderRow } from '../providers-model.ts';

function stateChildren(row: ProviderRow): (Node | string)[] {
  return [
...
class ProviderRowNodes {
  readonly item = h('li', { class: 'provider-row' });
  readonly tableRow = h('tr');
  private readonly name = h('span', { class: 'provider-name' });
  private readonly state = h('span', { class: 'status-cell' });
  private readonly p50Value = h('span', { class: 'p-value' });
  private readonly p95Value = h('span', { class: 'p-value' });
  private readonly plot = h('span', { class: 'latency-plot', 'aria-hidden': 'true' });
  private readonly bar = h('span', { class: 'latency-bar' });
  private readonly dot = h('span', { class: 'latency-dot' });
  private readonly tick = h('span', { class: 'latency-tick' });
  private readonly noSamples = h('span', { class: 'latency-plot is-empty' }, [
    h('span', { class: 'is-muted' }, ['No samples yet']),
  ]);
  private readonly meta = h('span', { class: 'provider-meta is-muted' });
  private readonly cells = Array.from({ length: 7 }, () => h('td'));
  private stateKey = '';
  ...
  constructor(id: string) {
    ...
    this.item.append(
      this.name,
      this.state,
      h('span', { class: 'provider-p' }, [h('span', { class: 'p-label is-muted' }, ['p50']), ' ', this.p50Value]),
      h('span', { class: 'provider-p' }, [h('span', { class: 'p-label is-muted' }, ['p95']), ' ', this.p95Value]),
      this.noSamples,
      this.meta,
    );
  ...
  update(row: ProviderRow): void {
    ...
    values.forEach((value, index) => {
      setText(this.cells[index] as HTMLElement, value);
    });
  }
}

export class ProvidersCard extends HTMLElement {
  private readonly heading = h('h1', { class: 'label-text' }, ['Providers']);
  private readonly summary = h('span', { class: 'providers-summary is-muted' });
  private readonly scale = h('span', { class: 'legend-scale' });
  private readonly list = h('ul', { class: 'providers-list' });
  private readonly axis = h('div', { class: 'providers-axis', 'aria-hidden': 'true' });
  ...
  private readonly rowNodes = new Map<string, ProviderRowNodes>();
  private summaryKey = '';
  private axisKey = '';
  ...
  showLoading(): void {
    replaceChildren(this, [
      ...
      h(
        'div',
        { class: 'providers-body', 'aria-busy': 'true', 'aria-label': 'Loading provider health' },
        [0, 1, 2, 3].map(() =>
          h('div', { class: 'skeleton-row provider-skeleton-row' }, [h('span', { class: 'skeleton skeleton-cell' })]),
        ),
      ),
    ]);
  }
  ...
  show(health: HealthSnapshot): void {
    const view = buildProvidersView(health);
    if (this.list.parentNode !== this) replaceChildren(this, this.content);
    ...
    const ordered: ProviderRowNodes[] = [];
    ...
    const inOrder =
      this.list.children.length === ordered.length &&
