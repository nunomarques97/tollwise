...
import type { RecentEntry } from '../api.ts';
import { h, replaceChildren } from '../dom.ts';
import { formatClock, formatMs } from '../format.ts';
import {
  attemptRows,
  candidateRows,
  costSection,
...
function renderRuns(runs: readonly SentenceRun[]): Node[] {
  return runs.map((run) => {
    if (run.strong === true) return h('b', {}, [run.text]);
    if (run.mono === true) return h('span', { class: 'is-mono' }, [run.text]);
...
export class RequestDrawer extends HTMLElement {
  private readonly scrim = h('div', { class: 'drawer-scrim', 'aria-hidden': 'true' });
  private readonly panel = h('div', {
    class: 'drawer-panel',
    role: 'dialog',
    'aria-modal': 'true',
    'aria-labelledby': 'drawer-title',
  });
  private readonly titleHeading = h('h2', { id: 'drawer-title', class: 'drawer-title', tabindex: '-1' });
  private readonly closeButton = h(
    'button',
    { type: 'button', class: 'pill-button', 'aria-label': 'Close request details' },
    ['Close'],
  );
  private readonly body = h('div', { class: 'drawer-body' });
  ...
  open(entry: RecentEntry, opener: HTMLElement, onClose: () => void): void {
    this.opener = opener;
  ...
  private onKeydown(event: KeyboardEvent): void {
    ...
    const focusable = [...this.panel.querySelectorAll<HTMLElement>(FOCUSABLE)];
  ...
  private render(entry: RecentEntry): void {
    ...
    const needsRow = h(
      'p',
      { class: 'chips' },
      needs.map((label) => h('span', { class: 'chip' }, [label])),
    );
    ...
    if (!this.panel.contains(this.body)) this.panel.append(this.body);
  }

  private renderStrip(entry: RecentEntry): HTMLElement {
    const items = routeStrip(entry);
    const nodes: Node[] = [];
    items.forEach((item, index) => {
      if (index > 0) nodes.push(h('li', { class: 'strip-sep', 'aria-hidden': 'true' }, ['→']));
      nodes.push(
        h('li', { class: `strip-item strip-${item.kind}` }, [
  ...
  private renderSubstitution(entry: RecentEntry): HTMLElement {
    const section = substitutionSection(entry);
    const body =
      section.kind === 'substituted'
        ? h('dl', { class: 'kv-list' }, [
            h('div', { class: 'kv-row' }, [
  ...
  private renderCost(entry: RecentEntry): HTMLElement {
    const section = costSection(entry);
    const items: Node[] = [];
    if (section.refused) {
      items.push(
        h('p', { class: 'note' }, ['No provider was called, so nothing was charged and there is no saving to report.']),
      );
    } else {
  ...
  private priceRow(label: string, price: PriceLine | undefined): HTMLElement {
    if (price === undefined) {
  ...
  private renderCandidates(entry: RecentEntry): HTMLElement {
      ...
      entry.selection === null
        ...
        : rows.length === 0
          ...
          : h(
              ...
              rows.map((row) =>
                h('li', { class: row.served ? 'is-served' : undefined }, [
  ...
  private renderExcluded(entry: RecentEntry): HTMLElement {
  ...
  private renderAttempts(entry: RecentEntry): HTMLElement {
