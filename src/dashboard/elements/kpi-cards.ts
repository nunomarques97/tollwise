...
import { type KpiCard, spokenValue } from '../overview-model.ts';

interface CardParts {
  readonly root: HTMLElement;
  readonly label: HTMLHeadingElement;
  readonly value: HTMLParagraphElement;
  readonly shown: HTMLSpanElement;
...
export class KpiCards extends HTMLElement {
  ...
  showLoading(): void {
    ...
    this.replaceChildren(
      ...[0, 1, 2].map(() =>
        h('div', { class: 'kpi-card', 'aria-hidden': 'true' }, [
  ...
  show(cards: readonly KpiCard[]): void {
    if (this.parts.length !== cards.length) this.build(cards.length);
    this.classList.remove('is-busy');
    cards.forEach((card, index) => {
      ...
      if (card.value.exact === undefined) part.value.removeAttribute('title');
      else part.value.title = card.value.exact;
