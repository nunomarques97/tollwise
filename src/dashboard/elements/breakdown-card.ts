...
import { type BreakdownView, keepExpanded, rowsToDraw } from '../charts/breakdown-model.ts';
...
export class BreakdownCard extends HTMLElement {
  private readonly headingId = `${TITLE_ID_PREFIX}${nextInstanceId()}`;
  private readonly heading = h('h2', { id: this.headingId, class: 'kpi-label' });
  ...
  private readonly rows = h('div', { class: 'breakdown-rows', role: 'img', tabindex: '-1' });
  private readonly footnote = h('p', { class: 'breakdown-footnote', hidden: true });
  private readonly showAll = h('button', { type: 'button', class: 'text-button', hidden: true });
  ...
  private shownRange: RangeId | undefined;
  private view: BreakdownView | undefined;
  private mono = false;
  private keyLabel = 'Provider';

  connectedCallback(): void {
    this.classList.add('card', 'breakdown-card');
    this.setAttribute('aria-labelledby', this.headingId);
    if (this.childElementCount === 0) this.showLoading('Spend by provider');
  ...
  show(title: string, view: BreakdownView, options: { readonly mono: boolean; readonly keyLabel: string }): void {
  ...
  private renderRows(view: BreakdownView): void {
