...
import { DEFAULT_RANGE, RANGES, type RangeId, rangeAfterKey } from '../ranges.ts';

export class RangeSelector extends HTMLElement {
  private selected: RangeId = DEFAULT_RANGE;
  private readonly buttons = new Map<RangeId, HTMLButtonElement>();
  ...
  set value(range: RangeId) {
    this.selected = range;
    this.render();
  }

  private choose(range: RangeId, moveFocus: boolean): void {
    const changed = range !== this.selected;
    this.selected = range;
    this.render();
    if (moveFocus) this.buttons.get(range)?.focus();
    if (changed) this.dispatchEvent(new CustomEvent<RangeId>('range-change', { detail: range, bubbles: true }));
  }
