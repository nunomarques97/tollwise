...
export function parseMicros(amount) {
...
function roundToUnits(micros, decimals) {
...
export function formatUsd(amount) {
    ...
    if (micros === null)
        return { text: 'Unknown', exact: undefined, unknown: true };
    const sign = micros < 0 ? '-' : '';
    const abs = Math.abs(micros);
