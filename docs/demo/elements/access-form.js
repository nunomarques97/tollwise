...
import { normalizeAccessKey } from '../access.js';
...
export class AccessForm extends HTMLElement {
    ...
    reveal = h('button', { type: 'button', class: 'field-button', 'aria-pressed': 'false' }, ['Show']);
    message = h('p', { id: MESSAGE_ID, class: 'field-error', hidden: true });
    submit = h('button', { type: 'submit', class: 'primary-button' }, ['Open dashboard']);
    form = h('form', { class: 'access-panel', novalidate: true, 'aria-labelledby': 'access-title' });
    connectedCallback() {
        ...
        this.form.addEventListener('submit', (event) => {
            ...
            this.dispatchEvent(new CustomEvent('access-key', { detail: key, bubbles: true }));
        });
        this.form.append(h('h1', { id: 'access-title', class: 'title-text' }, ['Enter the access key']), h('p', { class: 'access-intro' }, [
            'This Tollwise instance is protected by an access key. It is the value of ',
            h('code', {}, ['TOLLWISE_ACCESS_KEY']),
            ' where Tollwise runs.',
        ]), h('label', { for: INPUT_ID, class: 'field-label' }, ['Access key']), h('div', { class: 'field-row' }, [this.input, this.reveal]), this.message, h('p', { class: 'field-hint' }, [
