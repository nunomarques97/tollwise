...
import { h, prefersReducedMotion, replaceChildren, runs } from '../dom.js';
...
export class SavingsBlock extends HTMLElement {
    ...
    figure = h('p', { class: 'display-figure' });
    figureShown = h('span', { 'aria-hidden': 'true' });
    figureSpoken = h('span', { class: 'visually-hidden' });
    lead = h('p', { class: 'lead' });
    meter = h('div', { class: 'meter' });
    track = h('div', { class: 'meter-track', role: 'img' });
    spend = h('span', { class: 'meter-spend' });
    saved = h('span', { class: 'meter-saved' });
    legendSaved = h('span', { class: 'legend-text' });
    chips = h('div', { class: 'chips' });
    substitution = h('p', { class: 'substitution-line' });
    built = false;
    connectedCallback() {
    ...
    show(heading, view, animate) {
        ...
        if (view.figure.exact === undefined)
            this.figure.removeAttribute('title');
        else
            this.figure.title = view.figure.exact;
        ...
        replaceChildren(this.chips, view.chips.map((chip) => h('span', { class: chip.warning ? 'chip is-warning' : 'chip' }, runs(chip.runs))));
    ...
    build() {
        ...
        this.meter.replaceChildren(this.track, h('div', { class: 'meter-legend' }, [
