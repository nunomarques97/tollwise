...
import { h, prefersReducedMotion, replaceChildren } from '../dom.js';
import { announceDelay, shouldPrependLive, unseenRows, } from '../routing-model.js';
function pipsOf(routed) {
    if (routed.pips.length === 0)
        return undefined;
    return h('span', { class: 'pips', 'aria-hidden': 'true' }, routed.pips.map((pip) => h('i', { class: pip.ok ? 'pip is-ok' : 'pip is-failed' })));
}
/**
 ...
 */
function substitutionChip(mark) {
...
function routedCell(routed, mark) {
    if (routed.notRouted)
        return [h('span', { class: 'is-muted' }, ['Not routed'])];
    return [
...
function savedCellNodes(saved) {
    const amount = h('span', { class: saved.amount.unknown ? 'is-muted' : saved.positive ? 'is-positive-strong' : undefined }, [saved.amount.text]);
    if (saved.originText === undefined)
        return [amount];
...
export class RequestsTable extends HTMLElement {
    heading = h('h1', { class: 'label-text' }, ['Recent requests']);
    liveRegion = h('div', { class: 'visually-hidden', 'aria-live': 'polite' });
    newBar = h('div', { class: 'new-bar', hidden: true });
    newButton = h('button', { type: 'button', class: 'pill-button is-accent' });
    body = h('div', { class: 'requests-body' });
    count = h('span', { class: 'pager-count' });
    pagerButton = h('button', { type: 'button', class: 'pill-button' }, ['Load 50 older requests']);
    pagerError = h('p', { class: 'pager-error', hidden: true }, ['Could not load older requests.']);
    pagerEnd = h('span', { class: 'is-muted pager-end', tabindex: '-1', hidden: true }, [
        'That is every recorded request.',
    ]);
    pager = h('div', { class: 'pager', hidden: true }, [
    ...
    connectedCallback() {
        ...
        this.body.addEventListener('click', (event) => {
            ...
            if (button === null || requestId === undefined)
                return;
    ...
    flushPending(fromShowButton) {
        ...
        if (fade)
            for (const row of arrived)
                this.freshIds.add(row.requestId);
    ...
    rowElement(requestId) {
        return this.body.querySelector(`.request-row[data-request-id="${CSS.escape(requestId)}"]`);
    }
    timeButtonOf(requestId) {
        return this.body.querySelector(`.time-button[data-request-id="${CSS.escape(requestId)}"]`);
    }
    showLoading(narrow) {
        this.narrow = narrow;
        this.rows = [];
        ...
        replaceChildren(this.body, [0, 1, 2, 3, 4, 5, 6, 7].map(() => h('div', { class: 'skeleton-row' }, Array.from({ length: cells }, () => h('span', { class: 'skeleton skeleton-cell' })))));
    ...
    announce() {
        ...
        if (delay === 0)
            ...
            this.announceTimer = window.setTimeout(update, delay);
    }
    /**
     ...
     */
    insertRows(rows, where) {
        if (rows.length === 0)
            return;
        const container = this.body.querySelector(this.narrow ? 'ul.requests-list' : 'tbody');
        if (container === null) {
            this.render();
            return;
        }
    ...
    render() {
        ...
        const focusedId = active instanceof HTMLElement && this.body.contains(active) && active.classList.contains('time-button')
        ...
        if (focusedId !== undefined)
            this.timeButtonOf(focusedId)?.focus();
        if (this.pending.length > 0)
            this.renderNewBar();
    }
    tableRowNode(row) {
    ...
    renderList() {
        replaceChildren(this.body, [
            h('ul', { class: 'requests-list' }, this.rows.map((row) => this.listItemNode(row))),
        ]);
    }
    listItemNode(row) {
...
function rowClass(row, freshIds, lastOpenedId) {
    const classes = ['request-row'];
    if (freshIds.has(row.requestId))
        classes.push('is-fresh');
    if (row.requestId === lastOpenedId)
        classes.push('is-last-opened');
    return classes.join(' ');
}
