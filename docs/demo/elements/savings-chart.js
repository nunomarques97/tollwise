...
import { bucketLayout } from '../charts/layout.js';
import { formatAxisTick } from '../charts/scale.js';
import { hatchPattern, svgEl, svgText } from '../charts/svg.js';
import { spendGeometry } from '../charts/timeseries-model.js';
import { h, replaceChildren, runs } from '../dom.js';
import { chipsOf } from '../overview-model.js';
...
export class SavingsChart extends HTMLElement {
    heading = h('h1', { id: HEADING_ID, class: 'label-text' });
    savingsTitleWord = h('span', {});
    savingsTitleTotal = h('span', { class: 'chart-total' });
    readout = h('p', { class: 'chart-readout', 'aria-hidden': 'true' });
    savingsPlot = h('div', { class: 'chart-plot' });
    spendTitleWord = h('span', {});
    spendTitleTotal = h('span', { class: 'chart-total' });
    spendPlot = h('div', { class: 'chart-plot' });
    legend = h('div', { class: 'chart-legend' });
    ...
    connectedCallback() {
        ...
        if (this.childElementCount === 0)
            this.showLoading('minute');
        this.resizeObserver = new ResizeObserver(() => {
            if (this.view !== undefined)
                this.drawCharts(this.view);
        });
    ...
    show(heading, view, summary) {
        ...
        replaceChildren(this.chips, chipsOf(summary).map((chip) => h('span', { class: chip.warning ? 'chip is-warning' : 'chip' }, runs(chip.runs))));
    ...
    renderReadout(readout) {
        replaceChildren(this.readout, readout === undefined
            ? []
            : [
    ...
    applyHover(view, geometry, index) {
        this.hoverIndex = index;
        this.renderReadout(index === undefined ? view.readout : view.readouts[index]);
        for (const rule of this.querySelectorAll('.chart-hover-rule')) {
            if (index === undefined) {
     ...
     */
    easeNewestColumn(view, svg) {
        const newest = view.buckets.at(-1);
        const column = svg.querySelector('.chart-save.is-newest');
...
const RIGHT_PAD = 8;
function drawSavingsSvg(view, width, narrow) {
    ...
    const yFor = (value) => top + plotHeight - ((value - ticks.min) / span) * plotHeight;
    const zeroY = yFor(0);
    const svg = svgEl('svg', {
        class: 'chart',
    ...
    for (const bucket of view.buckets) {
        ...
        if (bucket.leftOut) {
            svg.append(svgEl('rect', { x, y: zeroY + 3, width: Math.max(layout.barWidth, 3), height: 4, class: 'chart-left-out' }));
        }
    }
...
function hoverRule(y1, y2) {
    return svgEl('line', { x1: 0, x2: 0, y1, y2, class: 'chart-hover-rule' });
}
function drawSpendSvg(view, width, narrow) {
    ...
    const yFor = (value) => top + plotHeight - (value / span) * plotHeight;
    const svg = svgEl('svg', {
        class: 'chart',
        viewBox: `0 0 ${width} ${height}`,
        width,
    ...
    if (line.d !== '')
        svg.append(svgEl('path', { d: line.d, class: 'chart-spend-line' }));
